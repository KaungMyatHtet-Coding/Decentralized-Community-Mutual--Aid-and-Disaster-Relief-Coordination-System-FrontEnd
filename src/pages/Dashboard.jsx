...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const [donationsRes, aidRes, notiRes] = await Promise.all([
        ...
        setStats({
          ...
          pendingAidRequests: aidRequests.filter((a) => a.status === "PENDING").length,
  ...
  const StatCard = ({ icon, label, value, color, sub }) => (
    <div className={`bg-slate-900/50 border ${color} rounded-2xl p-4 flex items-center gap-3`}>
      <span className="text-2xl">{icon}</span>
      <div>
        <p className="text-[10px] text-gray-500 uppercase tracking-wider font-mono">{label}</p>
        <p className="text-xl font-black text-white">
          {statsLoading ? <span className="text-slate-600 animate-pulse">—</span> : (value ?? 0)}
        </p>
      ...
      <div className="flex-1 p-6 max-w-6xl w-full mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-teal-950/20 border border-slate-800 p-6 rounded-2xl relative overflow-hidden">
          ...
          <h2 className="text-2xl font-extrabold text-white">
            Welcome Back, <span className="text-teal-400 font-black">{username}</span>! 👋
          </h2>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <StatCard icon="💰" label="Total Donations" value={stats?.totalDonations} color="border-teal-500/20" />
            <StatCard icon="⏳" label="Pending" value={stats?.pendingDonations} color="border-amber-500/20" sub="donations" />
            <StatCard icon="✅" label="Confirmed" value={stats?.confirmedDonations} color="border-emerald-500/20" sub="donations" />
            <StatCard icon="🆘" label="Aid Requests" value={stats?.totalAidRequests} color="border-rose-500/20" />
            <StatCard icon="🔄" label="Pending Aid" value={stats?.pendingAidRequests} color="border-orange-500/20" sub="requests" />
            <StatCard icon="🔔" label="Unread Notifs" value={stats?.unreadNotifs} color="border-purple-500/20" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl space-y-4 hover:border-slate-700 transition flex flex-col justify-between">
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-emerald-400">🆘 Aid Requests</h3>
              <p className="text-sm text-gray-400 leading-relaxed">
                View or create requests for emergency aid, supplies, and community support.
              </p>
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl space-y-4 hover:border-slate-700 transition flex flex-col justify-between">
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-teal-400">💰 Donation Campaigns</h3>
              <p className="text-sm text-gray-400 leading-relaxed">
                Support verified donation programs and contribute to funding people in need.
              </p>
            </div>
            <button
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl space-y-4 hover:border-slate-700 transition flex flex-col justify-between">
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-400 leading-relaxed">
                Join our team as a verified volunteer and participate in on-field aid activities.
              </p>
              {role === "ROLE_VOLUNTEER" && (
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl space-y-4 hover:border-slate-700 transition flex flex-col justify-between">
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-purple-400">📜 My Donations</h3>
              <p className="text-sm text-gray-400 leading-relaxed">
                View your full donation history, track status, and see confirmed contributions.
              </p>
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl space-y-4 hover:border-slate-700 transition flex flex-col justify-between">
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-orange-400">📰 News & Updates</h3>
              <p className="text-sm text-gray-400 leading-relaxed">
                Stay updated with the latest news and announcements from the platform.
              </p>
            </div>
            <button
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl space-y-4 hover:border-slate-700 transition flex flex-col justify-between">
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-cyan-400">📦 Item Donations</h3>
              <p className="text-sm text-gray-400 leading-relaxed">
                Donate physical items like food, clothing, or supplies to those in need.
              </p>
            </div>
            <button
              onClick={() => navigate("/item-donations")}
              ...
              {recentNotifs.map((n, i) => (
                <div
                  ...
                  className={`flex items-start gap-3 px-5 py-3.5 ${
                    i !== recentNotifs.length - 1 ? "border-b border-slate-800/50" : ""
